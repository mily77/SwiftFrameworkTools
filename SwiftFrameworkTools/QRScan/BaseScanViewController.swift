...
class BaseScanViewController: UIViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate {
    ...
    override open func viewDidAppear(_ animated: Bool) {
        ...
        perform(#selector(BaseScanViewController.startScan), with: nil, afterDelay: 0.3)
    ...
    @objc open func startScan()
    {
        ...
        if (scanObj == nil)
        {
            ...
            {
                cropRect = HSxScanView.getScanRectWithPreView(preView: self.view, style:scanStyle! )
            }
            ...
            scanObj = HSxScanWrapper(videoPreView: self.view,objType:arrayCodeType!, isCaptureImg: isNeedCodeImage,cropRect:cropRect, success: { [weak self] (arrayResult) -> Void in
    ...
    open func drawScanView()
    {
        if qRScanView == nil
        {
            qRScanView = HSxScanView(frame: self.view.frame,vstyle:scanStyle! )
     ...
     */
    open func handleCodeResult(arrayResult:[HSxScanResult])
    {
        if let delegate = scanResultDelegate  {
            
            self.navigationController? .popViewController(animated: true)
            let result:HSxScanResult = arrayResult[0]
            
            delegate.scanFinished(scanResult: result, error: nil)
            
        }else{
            
            for result:HSxScanResult in arrayResult
            {
    ...
    {
        HSxPermissions.authorizePhotoWith { [weak self] (granted) in
    ...
    public func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any])
    {
        ...
        if(image != nil)
        {
            let arrayResult = HSxScanWrapper.recognizeQRImage(image: image!)
