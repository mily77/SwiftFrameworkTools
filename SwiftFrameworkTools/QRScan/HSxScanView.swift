...
open class HSxScanView: UIView {
    //扫码区域各种参数
    var viewStyle:HSxScanViewStyle = HSxScanViewStyle()
    
    // 扫码区域
    var scanRetangleRect:CGRect = CGRect.zero
    
    // 线条扫码动画封装
    var scanLineAnimation:HSxScanLineAnimation?
    
    //网格扫码动画封装
    var scanNetAnimation:HSxScanNetAnimation?
     ...
     */
    public init(frame: CGRect, vstyle: HSxScanViewStyle) {
        var frameTmp = frame;
        ...
        case HSxScanViewAnimationStyle.LineMove:
            scanLineAnimation = HSxScanLineAnimation.instance()
            break
        case HSxScanViewAnimationStyle.NetGrid:
            scanNetAnimation = HSxScanNetAnimation.instance()
            break
        default:
            break
        }
    ...
    func startScanAnimation() {
        ...
        case HSxScanViewAnimationStyle.NetGrid:
            ...
            break
        case HSxScanViewAnimationStyle.LineStill:
    ...
    func drawScanRect()
    {
        ...
        {
        case HSxScanViewPhotoframeAngleStyle.Outer:
            diffAngle = linewidthAngle/3//框外面4个角，与框紧密联系在一起
            
        case HSxScanViewPhotoframeAngleStyle.On:
            diffAngle = 0
            
        case HSxScanViewPhotoframeAngleStyle.Inner:
    ...
    //根据矩形区域，获取识别区域
    static func getScanRectWithPreView(preView:UIView, style:HSxScanViewStyle) -> CGRect
    {
