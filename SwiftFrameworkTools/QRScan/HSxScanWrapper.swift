...
open class HSxScanWrapper: NSObject,AVCaptureMetadataOutputObjectsDelegate {
    ...
    //存储返回结果
    var arrayResult:[HSxScanResult] = [];
    
    //扫码结果返回block
    var successBlock:([HSxScanResult]) -> Void
    ...
    /**
     ...
     - returns:
     */
    init( videoPreView:UIView,objType:[AVMetadataObject.ObjectType] = [.qr],isCaptureImg:Bool,cropRect:CGRect=CGRect.zero,success:@escaping ( ([HSxScanResult]) -> Void) )
    {
        do{
        ...
        //        input = AVCaptureDeviceInput.init(device: device!)
    ...
    open func captureOutput(_ captureOutput: AVCaptureOutput!, didOutputMetadataObjects metadataObjects: [Any]!, from connection: AVCaptureConnection!) {
        ...
        for current:Any in metadataObjects
        {
            if (current as AnyObject).isKind(of: AVMetadataMachineReadableCodeObject.self)
            {
                ...
                arrayResult.append(HSxScanResult(str: codeContent, img: UIImage(), barCodeType: codeType.rawValue, corner: code.corners as [AnyObject]?))
            }
        }
    ...
    /**
     ...
     - returns: 返回识别结果
     */
    static open func recognizeQRImage(image:UIImage) ->[HSxScanResult]
    {
        var returnResult:[HSxScanResult]=[]
        
        if HSxScanWrapper.isSysIos8Later()
        {
            //if #available(iOS 8.0, *)
            ...
            if( features != nil && (features?.count)! > 0)
            {
                ...
                if feature.isKind(of: CIQRCodeFeature.self)
                {
                    ...
                    let result = HSxScanResult(str: scanResult, img: image, barCodeType: AVMetadataObject.ObjectType.qr.rawValue, corner: nil)
    ...
    static func getConcreteCodeImage(srcCodeImage:UIImage,codeResult:HSxScanResult)->UIImage?
    {
    ...
    //获取二维码的图像区域
    static open func getConcreteCodeRectFromImage(srcCodeImage:UIImage,codeResult:HSxScanResult)->CGRect
    {
